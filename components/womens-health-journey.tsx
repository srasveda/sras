...
import { HeartPulse, Leaf, MessageCircle, ShieldCheck, Sparkles } from "lucide-react";
...
const trustPoints = [
  ...
  },
  {
    ...
    copy: "Manufactured by Shri Ram Ayurvedshala with a consultation-first approach."
  }
];
...
export function WomensHealthJourney({ compact = false }: { compact?: boolean }) {
  return (
      ...
      <div className="container">
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            ...
            <h2 className="mt-3 font-serif text-4xl font-bold leading-tight md:text-5xl">
              Managing PCOS, hormonal imbalance and infertility with guided Ayurvedic support.
            </h2>
            <p className="mt-5 text-base leading-7 text-muted-foreground">
              Inspired by the SRAS women's health range, this journey helps customers start with their concern, understand the wellness focus, and consult before choosing a formulation.
            </p>
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Button asChild variant="whatsapp" size="lg">
                <a
                  href={whatsappUrl("Hi, I want guidance for PCOS, hormonal imbalance or fertility support.")}
                  target="_blank"
                  rel="noreferrer"
                >
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {trustPoints.map((point) => (
              <div key={point.title} className="rounded-xl border border-border bg-card p-5 shadow-line">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[1fr_1.1fr]">
            ...
            <div className="grid gap-6 p-6 md:p-8">
              <div className="flex items-center gap-4">
                <span className="relative flex h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-white/10">
                  <Image src="/images/brand/sras-logo-mark.png" alt="" fill sizes="56px" className="object-cover" />
                </span>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-secondary">Campaign range</p>
                  <h3 className="font-serif text-3xl font-bold">SRAS Women's Health Range</h3>
                </div>
              </div>
              <p className="text-sm leading-6 text-primary-foreground/80">
                A focused path for PCOS-related irregularities, hormonal wellness, uterine support, and reproductive care.
              </p>
              <div className="grid gap-2 text-sm">
                {["Trusted by doctors and retailers", "Manufactured by Shri Ram Ayurvedshala", "25+ years of Ayurvedic women's health experience"].map((item) => (
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            ...
            <div className="divide-y divide-border">
              {journeyRows.slice(0, compact ? 5 : journeyRows.length).map((row) => (
                <Link key={`${row.condition}-${row.support}`} href={row.href} className="grid grid-cols-[1fr_1fr_1fr] gap-px text-sm hover:bg-muted/60">
                  <div className="p-3 font-semibold">{row.condition}</div>
                  <div className="p-3 text-muted-foreground">{row.focus}</div>
                  <div className="p-3 font-semibold text-primary">{row.support}</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {productStory.map((item) => {
            ...
            return (
              <div key={item.slug} className="rounded-xl border border-border bg-card p-5 shadow-line">
                <div className={`relative aspect-[4/3] overflow-hidden rounded-lg ${product.tone}`}>
                  <Image src={product.image} alt={`${product.name} product`} fill sizes="(min-width: 1024px) 25vw, 50vw" className="object-contain p-4" />
                </div>
                ...
                <div className="mt-5 grid gap-2">
                  ...
                  <Button asChild variant="whatsapp" size="sm">
                    <a href={whatsappUrl(`Hi, I want guidance for ${product.name} in the women's health range.`)} target="_blank" rel="noreferrer">
        ...
        <p className="mt-6 text-center text-xs leading-5 text-muted-foreground">
          For wellness support and product education only. Please consult a qualified professional for diagnosis, treatment, pregnancy, chronic illness, or prescription medicine use.
        </p>
      </div>
    </section>
  );
